...
HierarchyApi.prototype.getFileNode = function (parameters) {
    ...
    var content = node.content,
        directoryFound = fileComponents.length === 1;
...
HierarchyApi.prototype.collectCartridges = function (parameters) {
    ...
    if (configuration.path !== '.') {
        ...
        // This cartridge collection refers to a subpath. Go there first.

        var pathSegments = configuration.path.split(path.sep);

        for (i = 0; i < pathSegments.length; i++) {
            if (investigationNode.content.hasOwnProperty(pathSegments[i])) {
                investigationNode = investigationNode.content[pathSegments[i]];
                pathFound = true;
            } else {
                pathFound = false;
            }
    ...
    if (pathFound) {
        ...
        // We have found the correct path to our cartridge

        var nodeIgnores = this.getNodeIgnores({
            node: investigationNode
        });

        var contentKeys = Object.keys(investigationNode.content);

        for (i = 0; i < contentKeys.length; i++) {

            var currentNode = investigationNode.content[contentKeys[i]];
