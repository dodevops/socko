...
function GeneratorApi(parameters) {
    ...
    for (i = 0; i < parameters.ignores.length; i++) {
    ...
    if (parameters.hasOwnProperty('renames')) {

        for (i = 0; i < parameters.renames.length; i++) {

            var rename = Rename.fromArgument({
                argument: parameters.renames[i]
            });

            if (!rename) {
                winston.error('Skipping rename.');
            } else {
    ...
    this.skipIdenticalSockets = false;

    if (parameters.hasOwnProperty('skipIdenticalSockets')) {
        this.skipIdenticalSockets = parameters.skipIdenticalSockets;
    }
...
GeneratorApi.prototype._handleFile = function (parameters) {
    ...
    fse.copySync(
        ...
        path.join(
            this.outputPath,
            this._rename({filepath: parameters.node.path})
        ),
        {
            'clobber': true
        }
...
GeneratorApi.prototype._handleSocket = function (parameters) {
    ...
    var newItemPath = path.join(
        ...
        this._rename({
            filepath: path.join(
                path.dirname(parameters.node.path),
    ...
    if (this.skipIdenticalSockets) {
        if (fse.pathExistsSync(newItemPath)) {
            var fileHash = crypto.createHash('sha256');
            fileHash.update(fs.readFileSync(newItemPath).toString());

            var contentHash = crypto.createHash('sha256');
            contentHash.update(fileContents);

            winston.debug('Generated socket file already exists. Checking for' +
                'differences');

            if (fileHash.digest('base64') !== contentHash.digest('base64')) {
                winston.debug('Files differ. Write new content');
                doWrite = true;
            } else {
                winston.debug('Files do not differ. Skip file');
            }
        } else {
            doWrite = true;
        }
        ...
        for (var a = 0; a < includeFileKeys.length; a++) {
            ...
            if (includeFiles.hasOwnProperty(includeFileKeys[a])) {
                ...
                fse.copySync(
                    ...
                    path.join(
                        this.outputPath,
                        this._rename({filepath: targetFile})
                    )
                );
...
/**
 * Walk through all renames and optionally rename an input path
 *
 * @param parameters
 * @param parameters.filepath relative file path
 * @returns {String} optionally renamed path
 * @private
 */

GeneratorApi.prototype._rename = function (parameters) {

    var returnString = parameters.filepath,
        found = false;

    for (var i = 0; i < this.renames.length; i++) {

        if (!found) {

            returnString = this.renames[i].translate({
                filepath: parameters.filepath
            });

            if (returnString !== parameters.filepath) {
                found = true;
            }
        }
    }
